...
describe('InMemoryRepository Unit Tests', () => {
    ...
    test('should bulk insert  entities', async () => {
        const entities = [
            ...
            new StubEntity({
                entity_id: new Uuid(),
                name: 'test 2 ', price: 200
            }),
            new StubEntity({
                entity_id: new Uuid(),
                name: 'test 3 ', price: 300
            })
        ];
        ...
        expect(repo.items[2]).toBe(entities[2]);
    });
    ...
    test("should be throws error on update when entity not found", async () => {
        const entity = new StubEntity({ name: "name value", price: 5 });
    ...
    test("should be update entity", async () => {
        const entity = new StubEntity({ name: "teste update", price: 5 });
        await repo.insert(entity);
